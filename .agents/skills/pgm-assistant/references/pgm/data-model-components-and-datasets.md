...
- `from_node` and `to_node` (branch3: all three nodes) may be identical; connecting a branch into itself is valid and no longer rejected by the validator (PGM v1.13.118). It behaves like a branch disconnected on one side.
